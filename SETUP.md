...
| `MONDAY_SIGNING_SECRET` | *(leave unset unless you have a monday **app**)* monday only signs a webhook delivery when the webhook was created through an app's **OAuth token**. Setup step 4 creates them with the personal API token, so they arrive **unsigned** and this secret has nothing to verify — `WEBHOOK_SECRET` is the lock that does the work. Set this only after moving the webhooks onto an app's OAuth token, and add `MONDAY_SIGNING_REQUIRED=true` to make unsigned deliveries a rejection. |
| `MONDAY_SIGNING_REQUIRED` | *(only with the above, and only on OAuth-registered webhooks)* `true` drops any delivery that arrives unsigned. On personal-token webhooks that is **every** delivery. |
...
| `RESEND_API_KEY` + `EMAIL_FROM` | set both (from resend.com → API Keys; `EMAIL_FROM` is a sender address on a domain you verified there) and the ⚠️ Check / ❌ Failed alert emails go out as real mail to the addresses saved on **Settings → Email notifications** — and, when its toggle is on there, the daily digest (yesterday's reads in one email, sent by the daily sweep). Unset, sends go to the logs only — safe for testing. Prefer plain SMTP instead? Set `SMTP_HOST` (+ optional `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`) with `EMAIL_FROM`. |
