...
-- ---------------------------------------------------------------------------
create table installer_accounts (
...
create index installer_accounts_match_key_idx on installer_accounts (match_key);
create index installer_accounts_active_idx on installer_accounts (active) where active;
...
create unique index eorder_ingests_dedupe_idx
  on eorder_ingests (opportunity_id, file_sha256)
  where opportunity_id is not null;

create index eorder_ingests_opp_idx on eorder_ingests (opportunity_id, created_at desc);
create index eorder_ingests_item_idx on eorder_ingests (monday_item_id);
...
create index portal_events_item_idx on portal_events (monday_item_id, created_at desc);
create index portal_events_account_idx on portal_events (installer_account_id, created_at desc);
...
create index jobs_cache_account_idx on jobs_cache (installer_account_id);
...
-- ---------------------------------------------------------------------------
create table unknown_order_reasons (
  id             uuid primary key default gen_random_uuid(),
...
create index unknown_order_reasons_reason_idx on unknown_order_reasons (order_reason);
